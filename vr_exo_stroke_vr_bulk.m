% VR_EXO experiment: BULK VR kinematic analysis of BILATERAL blocks (2,3,4)
% Current version coded on 2/14/23
% NOTE: This code is a partner to "vr_exo_kinematics_indiv.m". As of now
% , changes to this code must first be made in the 'indiv'
...
if strcmp(computer, 'PCWIN64')
    cd('C:\Users\brunfa01\Dropbox\Catholic U\VR_EXO_Stroke\Data')
    all_files = dir;
    is_dir = [all_files.isdir];
    dir_names = {all_files(is_dir).name};
    folders = dir_names(cellfun(@(x) strncmp(x, '2', 1), dir_names))';
    plist = importHandedness("C:\Users\brunfa01\Dropbox\Catholic U\VR_EXO_Stroke\Participants\VR_EXO_Stroke_Plist.xlsx");
else
    cd('/Users/alexbrunfeldt/Library/CloudStorage/Dropbox/Catholic U/VR_EXO_Stroke/Data')
    all_files = dir;
    is_dir = [all_files.isdir];
    dir_names = {all_files(is_dir).name};
    folders = dir_names(cellfun(@(x) strncmp(x, '2', 1), dir_names))';
    plist = importHandedness("/Users/alexbrunfeldt/Library/CloudStorage/Dropbox/Catholic U/VR_EXO_Stroke/Participants/VR_EXO_Stroke_Plist.xlsx");
end
%
...
for i = 1:length(folders)
    if strcmp(computer, 'PCWIN64')
        cd(['C:\Users\brunfa01\Dropbox\Catholic U\VR_EXO_Stroke\Data\', folders{i}])
        files = dir(['*uni*.xdf']);
        ...
        cd(['/Users/alexbrunfeldt/Library/CloudStorage/Dropbox/Catholic U/VR_EXO_Stroke/Data/', folders{i}])
        files_bil = dir(['*bil*.xdf']);
        files_exo = dir(['*exo*.xdf']);
        files = vertcat(files_bil, files_exo);
        for j = 1:length(files)
            file = files(j);
            pathname = file.folder;
            file = file.name;
...
data = load_xdf(fullfile(pathname, file));
...
for i = 1:length(targetHit)
   ...
   trialData(i).rtLH = (onsetLH(i) - indSpawn(i))/fs;
   trialData(i).rtRH = (onsetRH(i) - indSpawn(i))/fs;
end
...
% lab book pg 105). However, R code needs 54 trials per block in order to
...
if strcmp(computer, 'PCWIN64')
    cd('C:\Users\brunfa01\Dropbox\Catholic U\VR_EXO_Stroke\Data\post_process')
