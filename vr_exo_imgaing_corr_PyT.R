...
img.rms.loading = cbind(subset(emg.data.mbb, Subject %in% overlap_PyT$Subject & Condition == "Loading" & Muscle == "Impaired Deltoid"),
                        subset(overlap_PyT, select = -c(Subject)))
#
...
img.mc.loading = cbind(subset(emg.data.ratio.mbb, Subject %in% overlap_PyT$Subject & Condition == "Loading" & Muscle == "Deltoid"),
                        subset(overlap_PyT, select = -c(Subject)))
#
...
img.mc.loading %>%
  #filter(!Subject == 73872901)%>% 
  ggplot(aes(x = wCSTLL/1.5259022e-05, y = rmse.norm.bc.oc))+
  geom_point(size = 3)+
  geom_smooth(method = lm)+
  theme_cowplot(18)+
  xlab("wCSTLL") + ylab("\u0394 Muscle Contribution (%)")
...
img.to = cbind(subset(to.delt.df, subs %in% overlap_PyT$Subject),
                       subset(overlap_PyT, select = -c(Subject)))
#
...
# FM vs. Lesion Load---------------
if(!("FM" %in% colnames(overlap_PyT))){
overlap_PyT = cbind(overlap_PyT,p.table %>% filter(p.table$Subject.ID %in% overlap_PyT$Subject) %>% select('FM'))
}
...
lm.fm.slice = lm(FM ~ maxSliceWise_overlap, data = overlap_PyT)
summary(lm.fm.slice)
#
...
overlap_PyT %>%
  #filter(!Subject == 73872901)%>% 
  ggplot(aes(x = wCSTLL, y = FM))+ # the (/1.5259022e-05) thing is an artifact of the image processing (lesion marked as that number, not a binary 1/0)
  geom_point(size = 3)+
  geom_smooth(method = lm)+
  theme_cowplot(18)+
  xlab("wCSTLL (cc)") + ylab("UE-FM Score")

lm.fm.wcstll = lm(FM ~ wCSTLL, data = overlap_PyT)
